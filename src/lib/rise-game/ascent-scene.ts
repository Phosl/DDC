...
const ENEMY_KINDS: readonly EnemyKind[] = ['walker', 'charger', 'sentry', 'roller', 'mimic']
...
export function isDirectionalVerseAngle(angleDegrees: number) {
  ...
  const degreesFromUp = Math.abs(((((angleDegrees + 90 + 180) % 360) + 360) % 360) - 180)
...
export function resolveStablePlatformPosition({
  ...
  const x =
    minimumX <= maximumX ? Math.max(minimumX, Math.min(maximumX, platformX + offsetX)) : platformX
  return {
...
export function createAscentScene(
  ...
  class AscentScene extends PhaserRuntime.Scene implements AscentSceneHandle {
    ...
    private get actorScale() {
      return Math.max(1, bridge.actorScale || 1)
    }
    ...
    preload() {
      ...
      ;(['far', 'mid', 'near'] as const).forEach((layer) => {
        this.load.image(`background-giudecca-${layer}`, `/game/backgrounds/giudecca-${layer}.webp`)
      })
    }
    ...
    verifyCampaign() {
      ...
      CIRCLE_LEVELS.forEach((level, index) => {
        ...
        const boss = this.bosses
          .getChildren()
          .find((child) => isBodyObject(child) && child.getData('id') === level.boss)
    ...
    private finishActAssetLoad(actIndex: 1 | 2, bossId: BossId) {
      ...
      const boss = this.bosses
        .getChildren()
        .find((child) => isDynamicBodyObject(child) && child.getData('id') === bossId)
      ...
      sprite.setTexture(BOSS_TEXTURE, 0).setDisplaySize(96 * this.actorScale, 96 * this.actorScale)
      this.setWorldHitbox(sprite, 63 * this.actorScale, 63 * this.actorScale)
      const current = this.bossVisuals.get(sprite)
      current?.destroy()
      this.bossVisuals.set(
        sprite,
        createBossVisualController(this, sprite, bossId, this.actorScale),
      )
    }
        ...
        for (let column = 0; column < rowData.length; column += 1) {
          ...
          if (symbol === 'H' || symbol === 'V') {
            ...
            this.createEnemy(
              x,
              y - 10,
              ENEMY_KINDS[enemyCounter % ENEMY_KINDS.length],
              level.actIndex,
            )
            enemyCounter += 1
          } else if (symbol === 'F') {
    ...
    private createCircleMechanicVisuals(
      ...
      if (level.mechanics.includes('switches')) {
        ;[210, 520, 820].forEach((y, index) => {
          this.add
            .rectangle(index % 2 === 0 ? 34 : 350, offsetY + y, 10, 18, palette.accent, 0.9)
            .setDepth(-1)
        })
      ...
      if (level.mechanics.includes('gates') || level.mechanics.includes('tombs')) {
        ;[220, 510, 800].forEach((y) => {
          this.add
            .rectangle(GAME_WIDTH / 2, offsetY + y, 228, 54, palette.shadow, 0.46)
            .setDepth(-3)
          this.add
            .rectangle(GAME_WIDTH / 2, offsetY + y - 22, 218, 5, palette.accent, 0.5)
            .setDepth(-2)
        })
      ...
      if (level.mechanics.includes('rafts')) {
        ;[300, 620, 860].forEach((y) => {
          this.add
            .rectangle(GAME_WIDTH / 2, offsetY + y, GAME_WIDTH, 22, 0x39f4ff, 0.12)
            .setDepth(-5)
        })
        addLabel(18, 206, 'ZATTERE SULLO STIGE', '#39f4ff')
      }

      const spiralShift = level.orderFromBottom % 2 === 0 ? -24 : 24
      ;[180, 470, 760].forEach((y, index) => {
        const bandWidth = index === 1 ? 72 : 56
        const leftBand = this.add
          .rectangle(GAME_WIDTH / 2 + spiralShift - 118, offsetY + y, bandWidth, 220, palette.shadow, 0.24)
          .setDepth(13)
        const rightBand = this.add
          .rectangle(GAME_WIDTH / 2 - spiralShift + 118, offsetY + y, bandWidth, 220, palette.shadow, 0.24)
          .setDepth(13)
        this.spiralForeground.add(leftBand)
        this.spiralForeground.add(rightBand)
      })
    }
    ...
    private createLevelBackground(level: CircleLevelDefinition, offsetY: number) {
      ...
      if (
        !(['far', 'mid', 'near'] as const).every((layer) =>
          this.textures.exists(`background-${actKey}-${layer}`),
        )
      ) {
        return
      }
      ...
      ;(['far', 'mid', 'near'] as const).forEach((layer, layerIndex) => {
        ...
        tile
          ...
          .setData('spiralLayer', layerIndex)
          .setTileScale(transform.scaleX, transform.scaleY)
          .setTilePosition(transform.positionX, transform.positionY)
    ...
    private createPlatform(
      ...
      const falsePlatform =
        kind === 'crumble' && level.mechanics.includes('false-platforms') && row % 8 === 0
      const palette = ACT_PALETTES[actIndex]
      const isOneWay = kind === 'one-way'
      const height = isOneWay ? 8 : 14
      const color = kind === 'crumble' ? palette.accent : palette.platform
      const authoredVisual = this.textures.exists('platform-v2') && width >= 32
      const fillAlpha = authoredVisual ? 0.2 : isOneWay ? 0.86 : 0.95
      const strokeAlpha = authoredVisual ? (isOneWay ? 0.72 : 0.4) : 1
      const strokeColor = isOneWay ? palette.accent : palette.shadow
    ...
    private createMovingPlatform(x: number, y: number, actIndex: 0 | 1 | 2, axis: 'x' | 'y') {
      ...
      platform.setData({
        ...
        range: axis === 'x' ? platformTuning.horizontalRange : platformTuning.verticalRange,
        lastX: x,
        lastY: y,
    ...
    private createEnemy(x: number, y: number, kind: EnemyKind, actIndex: 0 | 1 | 2) {
      ...
      if (authored) {
        const visual = createEnemyVisualController(this, enemy, kind, this.actorScale)
      ...
      if (authored) {
        const targetWidth = (kind === 'flyer' ? 26 : 22) * this.actorScale
        const targetHeight = (kind === 'flyer' ? 18 : 28) * this.actorScale
        body.setSize(
    ...
    private createBoss(
      ...
      if (authored) {
        const visual = createBossVisualController(this, boss, id, this.actorScale)
      ...
      if (authored) this.setWorldHitbox(boss, 63 * this.actorScale, 63 * this.actorScale)
    ...
    private createPlayer(x: number, y: number) {
      ...
      this.player = this.physics.add.sprite(
        x,
        y,
        this.authoredPlayer ? PLAYER_TEXTURE : 'player-fallback',
        0,
      )
      ...
      if (this.authoredPlayer) {
        this.playerVisual = createPlayerVisualController(this, this.player, this.actorScale)
    ...
    private createCollisions() {
      ...
      this.physics.add.collider(this.enemies, this.oneWayPlatforms, undefined, (enemy, platform) =>
        this.shouldCollideOneWay(enemy, platform),
      )
      ...
      ;[
        ...
        this.physics.add.collider(this.hostileProjectiles, platforms, (projectile) => {
    ...
    private resolveLastStableSpawn() {
      ...
      if (platform?.active && platform.body?.enable && offset) {
    ...
    private shouldCollideOneWay(
      ...
      const isFirstCircle =
        ...
        platform.body.width >= 96 &&
        (platform.getData('row') === 6 || platform.getData('row') === 3) &&
        body.velocity.y < 0

      if (isFirstCircle) {
        const tolerancePx = this.tuning.platforms.oneWayTolerance + 10
        if (
          body.right > platform.body.left + 8 &&
          body.left < platform.body.right - 8 &&
          body.bottom <= platform.body.top + tolerancePx
        ) {
          body.setVelocityY(0)
      ...
      const platformVelocityY =
        platform.body instanceof PhaserRuntime.Physics.Arcade.Body ? platform.body.velocity.y : 0
    ...
    private updateCrumblePlatformBar(platform: BodyObject) {
      ...
      const restoreAt = platform.getData('crumbleRestoresAt') as number | null
      ...
      const width = Math.max(22, platform.width * 0.76)
      const height = 4
      const x = platform.x - width / 2
      const y = platform.y - (platform.displayHeight + 8)
      const fill = elapsed <= delayMs ? 1 - elapsed / delayMs : Math.min(1, (elapsed - delayMs) / restoreMs)
      const fillColor = elapsed <= delayMs ? 0xffcf5e : 0x39f4ff

      bar.setVisible(true)
      bar.clear()
      bar.fillStyle(0x0e1a2d, 0.9)
      bar.fillRect(x, y, width, height)
      bar.lineStyle(1, 0x7ca9c6, 0.7)
      bar.strokeRect(x, y, width, height)
      bar.fillStyle(fillColor, 1)
      bar.fillRect(x + 1, y + 1, Math.max(0, (width - 2) * fill), height - 2)
    }
    ...
    private updateMovingPlatforms(delta: number) {
      ...
      this.movingPlatforms.getChildren().forEach((child) => {
        ...
        if (child.getData('tuningMode') !== tuningMode) {
          ...
          child.setData(
            'range',
            axis === 'x' ? platformTuning.horizontalRange : platformTuning.verticalRange,
          )
          child.setDisplaySize(platformTuning.movingWidth, 12)
          child.body.setSize(platformTuning.movingWidth, 12, true)
          const authoredVisual = child.getData('visual') as Phaser.GameObjects.Image | undefined
          authoredVisual?.setDisplaySize(platformTuning.movingWidth, 24)
        }
        const origin =
          axis === 'x' ? (child.getData('originX') as number) : (child.getData('originY') as number)
        const coordinate = axis === 'x' ? child.x : child.y
        const range = child.getData('range') as number
        const body = child.body
        const speed = axis === 'x' ? platformTuning.horizontalSpeed : platformTuning.verticalSpeed
        ...
        const velocity = delta > 0 ? ((next.position - coordinate) * 1_000) / delta : 0
    ...
    private updatePlayer(time: number, delta: number) {
      ...
      if (mechanics.includes('ice') && (body.blocked.down || body.touching.down)) {
        body.setVelocityX(
          PhaserRuntime.Math.Linear(body.velocity.x, targetSpeed, mechanicTuning.iceResponsiveness),
        )
      } else {
        body.setVelocityX(targetSpeed)
      }
      if (mechanics.includes('wind') && !body.blocked.down) {
        body.setVelocityX(body.velocity.x + Math.sin(time / 410) * mechanicTuning.windForce)
      }
      if (mechanics.includes('knockback') && !body.blocked.down) {
        body.setVelocityX(body.velocity.x + Math.sin(time / 260) * mechanicTuning.knockbackForce)
      }
    ...
    private updateAimReticle(aim: Readonly<{x: number; y: number}> | null | undefined) {
      ...
      this.aimReticle
        .setPosition(this.player.x + unitX * 38, this.player.y - 31 + unitY * 38)
        .setVisible(true)
    }

    private updatePlayerAnimation(time: number, grounded: boolean) {
      if (!this.authoredPlayer || !this.playerVisual) return
      const fireVisualWindowMs = Math.max(150, this.tuning.combat.fireIntervalMs - 20)
      const RUN_VELOCITY_THRESHOLD = 24
      const body = this.player.body as Phaser.Physics.Arcade.Body
      const firing = time - this.lastShotAt < fireVisualWindowMs
    ...
    private tryFire(time: number) {
      ...
      const angles =
        time < this.rimaUntil ? [baseAngle - 10, baseAngle, baseAngle + 10] : [baseAngle]
      ...
      angles.forEach((angle) => {
        ...
        if (recipe)
          projectile.setDisplaySize(
            recipe.recommendedDisplaySize.width,
            recipe.recommendedDisplaySize.height,
          )
        projectile
          .setDepth(11)
    ...
    private updateEnemies(time: number) {
      ...
      this.enemies.getChildren().forEach((child) => {
        ...
        const engaged = this.isActorEngaged(child.y, enemyTuning.engagementDistance)
        if (!engaged) {
        ...
        if (kind === 'flyer') {
          ...
          if (!child.getData('engaged')) {
            ...
            child.setData('nextAttackAt', time + enemyTuning.sentry.firstAttackDelayMs)
          }
          const nextAttackAt = child.getData('nextAttackAt') as number
          if (time >= nextAttackAt) {
            child.setData('nextAttackAt', time + enemyTuning.sentry.intervalMs)
            ...
            this.time.delayedCall(enemyTuning.sentry.windupMs, () => {
              ...
              if (
                ...
                !this.isActorEngaged(child.y, this.tuning.enemies.engagementDistance)
              ) {
                return
              }
          ...
          if (
            Math.abs(child.x - originX) > (kind === 'roller' ? 96 : 56) ||
            body.blocked.left ||
            body.blocked.right
          ) {
    ...
    private updateBosses(time: number) {
      this.bosses.getChildren().forEach((child) => {
        ...
        const engaged = this.isActorEngaged(child.y, bossTuning.engagementDistance)
        ...
        if (!active && engaged) {
          ...
          this.bossCheckpointSpawn = this.resolveLastStableSpawn() ?? circleSpawn?.clone() ?? null
          this.breath = this.tuning.combat.maxBreath
        ...
        if (!engaged) {
          ...
          child.setData(
            'nextAttackAt',
            Math.max(child.getData('nextAttackAt') as number, time + bossTuning.firstAttackDelayMs),
          )
          return
        }
        if (!child.getData('engaged')) {
        ...
        if (Math.abs(child.x - originX) > 145)
          child.x = originX + Math.sign(child.x - originX) * 145
        ...
        if (time >= nextAttackAt) {
          ...
          this.time.delayedCall(bossTuning.telegraphMs, () => {
            ...
            if (
              ...
              !this.isActorEngaged(child.y, this.tuning.bosses.engagementDistance)
            ) {
              child.setData('nextAttackAt', this.elapsedMs + this.tuning.bosses.firstAttackDelayMs)
              return
            }
    ...
    private fireHostileProjectile(
      ...
      const direction = new PhaserRuntime.Math.Vector2(targetX - x, targetY - y)
        .normalize()
        .scale(speed)
    ...
    private updateCamera() {
      ...
      const spiralProgress = PhaserRuntime.Math.Clamp((WORLD_HEIGHT - this.player.y) / WORLD_HEIGHT, 0, 1)
      const spiralOffset = Math.sin(spiralProgress * Math.PI * 5.5) * 16 + Math.sin(spiralProgress * Math.PI * 2) * 5
      const cameraCenterX = this.worldGeometry.left + this.worldGeometry.width / 2
      const desiredX = PhaserRuntime.Math.Clamp(
        cameraCenterX + spiralOffset * 0.55 - this.viewportWidth / 2,
        this.worldGeometry.left,
        Math.max(this.worldGeometry.left, this.worldGeometry.right - this.viewportWidth),
      )
      this.cameras.main.scrollX = desiredX
      this.backgroundTiles.forEach((tile) => {
        const layerIndex = (tile.getData('spiralLayer') as number | undefined) ?? 0
        tile.setX(WORLD_WIDTH / 2 + spiralOffset * (0.18 + layerIndex * 0.12))
        tile.setAlpha([0.5, 0.46, 0.34][layerIndex] + Math.abs(spiralOffset) * 0.001)
      })
      let spiralBandIndex = 0
      this.spiralForeground.forEach((band) => {
        const depthBias = spiralBandIndex % 2 === 0 ? -1 : 1
        band.setX(WORLD_WIDTH / 2 + spiralOffset * 0.28 * depthBias)
        band.setAlpha(0.18 + Math.abs(spiralOffset) * 0.003)
        spiralBandIndex += 1
      })
    }
    ...
    private collectPickup(pickup: BodyObject) {
      ...
      if (kind === 'voice') {
        ...
        this.breath = restoreBreath(this.breath, 46, this.tuning.combat.maxBreath)
      } else if (kind === 'rima') {
    ...
    private damageEnemy(enemy: BodyObject) {
      ...
      if (hp <= 0) {
        ...
        this.time.delayedCall(70, () => {
          if (enemy.active && 'clearTint' in enemy)
            (enemy as Phaser.Physics.Arcade.Sprite).clearTint()
        })
      }
    }
    ...
    private takeDamage(reason: string, isFall = false) {
      ...
      if (this.shield) {
        this.shield = false
        this.invulnerableUntil = this.elapsedMs + this.tuning.player.invulnerableMs
    ...
    private getDamageRespawnSpawn() {
      ...
      if (this.bossCheckpointSpawn && this.bossCheckpointCircleIndex === this.currentCircleIndex) {
    ...
    private clearThreatsNearRespawn() {
      ...
      this.enemies.getChildren().forEach((child) => {
        if (!isBodyObject(child)) return
        if (
          PhaserRuntime.Math.Distance.Between(child.x, child.y, spawn.x, spawn.y) <
    ...
    private completeRun() {
      ...
      this.emitAnnouncement(
        isRecord ? 'Nuovo record. Le stelle ricordano il tuo tempo.' : this.statusText,
      )
      this.emitSnapshot(true)
    }
